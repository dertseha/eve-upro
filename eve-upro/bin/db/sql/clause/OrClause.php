...
/**
 * OR operator; Takes at least two subclauses, puts them in brackets and combines them with OR
 */
class OrClause extends \upro\db\sql\clause\AbstractClause
{
   ...
   public function toSqlText(\upro\db\sql\SqlDictionary $dict)
   {
      ...
      {
         $result = $result->append($this->subClauses[$i]->toSqlText($dict), ')' . $dict->getOr() . '(');
      }
